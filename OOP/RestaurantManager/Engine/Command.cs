namespace RestaurantManager.Engine
{
    ...
    public class Command : ICommand
    {
        ...
        private void TranslateInput(string input)
        {
            ...
            var parametersKeysAndValues = input.Substring(
                parametersBeginning + 1, input.Length - parametersBeginning - 2)
                .Split(
                new[] { CommandParameterSeparator }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var parameter in parametersKeysAndValues)
            {
                var split = parameter.Split(
                    new[] { CommandValueSeparator }, StringSplitOptions.RemoveEmptyEntries);
